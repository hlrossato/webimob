...
username = 'webimob'
host = '50.116.39.155'
repositorio = 'git@bitbucket.org:hlrossato/webimob.git'
...
def cw():
    ...
    local('compass watch webimob/static')

def compass_compile():
    """compila o compassa remoto"""
    log('Compilando arquivos SASS')
    with cd(env.project_path):
        run('compass compile -e production webimob/static')
...
def translate():
    ...
    local('django-admin.py makemessages --locale=en --ignore=templates/admin --ignore=webimob/settings.py')
